...
collision: alarm.hasOwnProperty('collision') ? alarm.collision : null,
  ...
  const startLocalSSEStream = useCallback(() => {
    if (localEventSourceRef.current) {
      console.log('⚠️ Local SSE stream already active');
      return;
    }

    try {
      console.log('🚀 Starting local SSE stream...');
      const token = getAuthToken();

      if (!token) {
        console.error('❌ No authentication token for local SSE');
        return;
      }

      const sseUrl = `http://164.52.194.198:9090/alarm/v1/manager/stream?token=${encodeURIComponent(token)}`;
      localEventSourceRef.current = new EventSource(sseUrl);

      localEventSourceRef.current.onopen = () => {
        console.log('✅ Local SSE stream connected');
        setIsLocalStreamConnected(true);
      };

      localEventSourceRef.current.onmessage = (event) => {
        try {
          const alarmData = JSON.parse(event.data);
          console.log('📡 Raw SSE data:', alarmData);

          // Avoid duplicate processing
          if (seenLocalAlarmIds.current.has(alarmData.alarmId)) {
            console.log('⚠️ Duplicate local alarm ignored:', alarmData.alarmId);
            return;
          }
          seenLocalAlarmIds.current.add(alarmData.alarmId);

          const newAlarm = normalizeAlarm(alarmData, false);
          newAlarm.source = 'local_sse';
          newAlarm.isLive = true;

          // Add new alarm and trim to 500
          setAlarms(prevAlarms => [newAlarm, ...prevAlarms].slice(0, 500));
          triggerAlarmNotification(newAlarm);
          console.log(`🔔 New local alarm added: ${newAlarm.alarmId}`);
        } catch (error) {
          console.error('❌ Error processing local SSE alarm:', error);
        }
      };

      localEventSourceRef.current.onerror = (error) => {
        console.error('❌ Local SSE stream error:', error);
        ...
        setTimeout(() => {
          if (localStreamEnabled) {
            console.log('🔄 Attempting to reconnect local SSE stream...');
            startLocalSSEStream();
          }
        }, 5000);
      };
      ...
      console.error('❌ Failed to start local SSE stream:', error);
      setIsLocalStreamConnected(false);
    }
  }, [localStreamEnabled, soundEnabled]);
